...
const SINGLE_FIELD_KEYS = ["command", "file_path", "path", "query", "pattern"];
...
function locateMessages(data: unknown): unknown[] {
  if (isObject(data)) {
...
function normalizeContent(content: unknown): unknown[] {
  ...
  return [{ type: "unknown", text: String(content) }];
}
...
function stringifyToolInput(inputObject: unknown): string {
  if (typeof inputObject === "string") return inputObject;
  if (isObject(inputObject)) {
    const present = SINGLE_FIELD_KEYS.filter((key) => key in inputObject);
    if (present.length === 1) return String(inputObject[present[0]]);
  }
  return stringifyJson(inputObject);
}

function stableEntryKey(entry: ChatEntry): string {
...
function reindexEntries(entries: ChatEntry[]): ChatEntry[] {
  return entries.map((entry, index) => ({ ...entry, index }));
}

function flattenToolResultContent(content: unknown): string {
  if (content == null) return EMPTY_TOOL_RESULT_TEXT;
  if (typeof content === "string") return cleanText(content) || EMPTY_TOOL_RESULT_TEXT;
  if (Array.isArray(content)) {
    const parts = content.map((item) => {
      if (typeof item === "string") return cleanText(item);
      if (isObject(item)) return cleanText(item.text || item.thinking || stringifyJson(item));
      return cleanText(String(item));
    });
    return cleanText(parts.join("\n")) || EMPTY_TOOL_RESULT_TEXT;
  }
  return cleanText(stringifyJson(content));
}
...
function flattenBlock(
  ...
  const block = typeof blockValue === "string" ? { type: "text", text: blockValue } : blockValue;
...
export function parseTraceClient(data: unknown): ChatEntry[] {
  ...
  messages.forEach((message, messageIndex) => {
    ...
    try {
      const { role, rawRole } = normalizeRole(message.role);
...
function parseOptionalOutput(outputData: unknown | null): [ChatEntry[], string[]] {
  if (outputData == null) return [[], ["Output JSON was empty or not provided; showing input transcript only."]];
  return [parseTraceClient(outputData), []];
}

export function parseCaptureClient(inputData: unknown, outputData: unknown | null): ClientParseCaptureResult {
  const entries = parseTraceClient(inputData);
