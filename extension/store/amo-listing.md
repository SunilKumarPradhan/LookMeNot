...
Linter (`npm run lint:firefox`, i.e. `web-ext lint`): 0 errors, 2 warnings. Both are `UNSAFE_VAR_ASSIGNMENT` inside the bundled react-dom runtime (a constant `<script>` probe string and the `dangerouslySetInnerHTML` helper). The extension's own source does not assign to `innerHTML` and does not use `dangerouslySetInnerHTML`.
