...
import { OPEN_PANEL_EVENT, panelBus } from "../core/panelBus";
...
export function ExtensionApp({ initialOpen }: ExtensionAppProps) {
  ...
  // Scan once on load, then only when the page DOM changes (debounced, and never while the tab is hidden).
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      mutationTimer = window.setTimeout(() => {
        mutationTimer = null;
        if (!document.hidden) runPageScan();
      }, 1500);
    });
  ...
  // Trace JSON forwarded by the page bridge (same origin, same tab).
  useEffect(() => {
  ...
  // Toolbar-button request relayed by the content script.
  useEffect(() => {
    const onOpenRequest = () => {
    ...
    panelBus.addEventListener(OPEN_PANEL_EVENT, onOpenRequest);
    return () => panelBus.removeEventListener(OPEN_PANEL_EVENT, onOpenRequest);
  }, [runPageScan]);
