...
export function detectLangfusePage(): boolean {
  ...
  if (/\/(project|projects|trace|traces|sessions|observations)\b/.test(path) && /langfuse/i.test(document.title)) return true;
  const bodyText = document.body?.innerText?.slice(0, 4000) ?? "";
  return /langfuse/i.test(bodyText) && /\b(trace|traces|observations|input|output)\b/i.test(bodyText);
}
